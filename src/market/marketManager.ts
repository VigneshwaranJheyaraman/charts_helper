 ...
 * @typedef {(symbol:ChartSymbol) => Array<MarketRule>} RulesFactory
 */
...
export default class MarketManager extends AbstractCompose<MarketManageProps> implements IMarketManager{
    ...
    /**
     * Returns a MarketManager instance
     * @param {MarketManagerProps} props - constructor properties
     */
    constructor(props:MarketManageProps){
        super(props);
        let symbolSubscription:Subscription = this.__subscription.subscribe(ChartSymbol.SubscriptionTopic, (changeEvent:SubscriptionEvent) => {
            if(changeEvent.data){
    ...
    /**
     * Returns if the date specified is a valid market day without considering the market's active status
    ...
    /**
     * Returns boolean to check if the date is a valid market day and the time is a valid market session time
    ...
    /**
     * Backtrack to a valid market date from current date to a number of ticks
