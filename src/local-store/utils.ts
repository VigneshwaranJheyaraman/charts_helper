 ...
 * @method encodeDataForLocalStore
 * @description Logic to encode a string for saving it in local store to avoid debugging
  ...
  return stringToBeEncoded
    ...
    .map((char: string) => {
      ...
      return `${String.fromCharCode(
        encodedValue % ASCII_LEN
      )}${randomNumber}${String.fromCharCode(Math.floor(encodedValue / 255))}`;
    })
    .join("");
}
/**
 * @category Store-Utils
 * @method decodeDataFromLocalStore
 * @description Logic to decode the encoded string
...
export function decodeDataFromLocalStore(stringToBeDecoded: string): string {
  ...
  let setOfThreeString: RegExpMatchArray | null =
    stringToBeDecoded.match(/\.{1,3}/g);
  return setOfThreeString
    ? setOfThreeString
        .map((remNumQuo: string) => {
          let [remainder, _, quotient]: number[] = remNumQuo
            .split("")
            .map((char: string) => char.charCodeAt(0));
          return `${String.fromCharCode(
            (ASCII_LEN * quotient + remainder) / key
          )}`;
        })
        .join("")
    : "";
