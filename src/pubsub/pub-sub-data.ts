...
/**
 * @category Pub-Sub-Utils
 * @method subscribe
 * @description Subscribe to a token
...
export function subscribe(token: string, subscriber: Subscriber): void {
  let subscriptionList: Array<Subscriber> | undefined =
    allPublisherSubscribers.get(token) || [];
...
export function unsubscribe(
  ...
  removeAllSubscriptions: boolean = false
): void {
  let subscriptionList: Array<Subscriber> | undefined =
    allPublisherSubscribers.get(token) || [];
  subscriptionList = subscriptionList.filter(
    (sub: Subscriber) => sub.toString() !== subscriber.toString()
  );
 ...
 * @description Get list of Subscriber for a given token
